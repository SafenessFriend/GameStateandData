...
void GridApp::findShortestPath()
{
	...
	// set up the 'vectors'
	typedef std::pair <Point, float> iPath;
	vector<Point> explored; //where we have looked
	std::list<iPath> connectedQueue; //w what are we currently looking at
	...
	if (PRINTOUT_PATH)
		cout << "Starting point: ( " << m_startPoint.x << " , " << m_startPoint.y << " ) Finishing: ( " << m_stopPoint.x << " , " << m_stopPoint.y << " ) " << endl;

	Point nextPoint;
	for (unsigned int edge = 0; edge < MAX_EDGES; edge++)
	{
		unsigned int edgecost = m_travel[m_startPoint.x][m_startPoint.y].edges[edge];
	...
	while (true)
	{
		...
		for (unsigned int edge = 0; edge < MAX_EDGES; ++edge)
		{
			...
			if (totalCost < m_travel[nextPoint.x][nextPoint.y].costFromStart)
			{
				...
				if (PRINTOUT_PATH)
					cout << "added : ( " << nextPoint.x << " , " << nextPoint.y << " ) tp the queue cost = " << totalCost << endl;
			}
		}
