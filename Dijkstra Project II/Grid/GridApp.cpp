...
void GridApp::findShortestPath()
{
	...
	vector<Point> explored;		// where have we looked
	std::list<iPath> connectedQueue;	// what are we currently looking at
	...
	if (PRINTOUT_PATH)
		...
		" ) Finishing: ( " << m_stopPoint.x << " , " << m_stopPoint.y <<
		" ) " << endl;
	// setup the queue
	Point nextPoint;
	for (unsigned int edge = 0; edge < MAX_EDGES; edge++) {
	...
	while (true) {
		...
		if (PRINTOUT_PATH)
			cout << "Point selected is: ( " << nextPoint.x << 
			" , " << nextPoint.y << " ) " << endl;
		...
		for (unsigned int edge = 0; edge < MAX_EDGES; ++edge) {
			...
			if (totalCost < m_travel[nextPoint.x][nextPoint.y].costFromStart) {
				...
				if (PRINTOUT_PATH)
					cout << "added: ( " << nextPoint.x <<
					" , " << nextPoint.y << " ) to the queue cost = " << totalCost << endl;
			}
		}
		// sort the queue
		connectedQueue.sort([](const iPath &a, const iPath &b) { return a.second < b.second;  });
